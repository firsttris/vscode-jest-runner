...
describe('TestArgumentBuilder', () => {
  ...
  describe('deno', () => {
    ...
    it('should NOT include filter when running whole file', () => {
      const files = ['/path/to/test.ts'];
      const testsByFile = new Map();
      testsByFile.set('/path/to/test.ts', [
        { label: 'test1' },
        { label: 'test2' },
      ]);
  ...
  describe('vitest', () => {
    it('should generate correct args for vitest with coverage', () => {
      ...
      const testsByFile = new Map();
      testsByFile.set('/path/to/test.ts', [{ label: 'test1' }]);
      (mockConfig.getVitestConfigPath as jest.Mock).mockReturnValue(
        '/path/to/vitest.config.ts',
      );
    ...
    it('should use partial run logic when appropriate', () => {
      ...
      (mockConfig.buildVitestArgs as jest.Mock).mockReturnValue([
        '--mocked-vitest-args',
      ]);
  ...
  describe('playwright', () => {
    ...
    it('should generate correct args for playwright full file run', () => {
      const files = ['/path/to/test.spec.ts'];
      const testsByFile = new Map();
      testsByFile.set('/path/to/test.spec.ts', [
        { label: 'test1' },
        { label: 'test2' },
      ]);
  ...
  describe('jest', () => {
    it('should generate correct args for jest with coverage', () => {
      ...
      const testsByFile = new Map();
      testsByFile.set('/path/to/test.ts', [{ label: 'test1' }]);
      (mockConfig.getJestConfigPath as jest.Mock).mockReturnValue(
        '/path/to/jest.config.ts',
      );
    ...
    it('should resolve nested interpolation placeholders for partial jest runs', () => {
      const files = ['/path/to/test.ts'];
      const testsByFile = new Map();
      testsByFile.set('/path/to/test.ts', [
        { label: 'should run correctly for id ${test_case.id}' },
      ]);
      ...
      expect(mockConfig.buildJestArgs).toHaveBeenCalledWith(
        '/path/to/test.ts',
        'should run correctly for id (.*?)',
        true,
        expect.any(Array),
      );
    });

    it('should prefer full test pattern from TestItem id over short label', () => {
      const files = ['/path/to/test.ts'];
      const testsByFile = new Map();
      testsByFile.set('/path/to/test.ts', [
        {
          label: 'b',
          id: '/path/to/test.ts:it:7:nested a b',
        },
      ]);
