...
describe('TestRunnerConfig - Node Test Runner', () => {
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockReturnValue(
        new TextEditor(
          new Document(new Uri('/home/user/project/test.test.js') as any),
        ) as any,
      );
  });
  ...
  describe('getTestFramework', () => {
    ...
    it('should use active editor when no path provided', () => {
      jest
        .spyOn(vscode.workspace, 'getConfiguration')
        .mockReturnValue(new WorkspaceConfiguration({}));
      const mockGetFramework = jest
        .spyOn(testFileDetection, 'getTestFrameworkForFile')
        .mockReturnValue('node-test');

      config.getTestFramework();

      expect(mockGetFramework).toHaveBeenCalledWith(
        '/home/user/project/test.test.js',
      );
    });
  });
  ...
  describe('nodeTestRunOptions', () => {
    ...
    it('should return configured options', () => {
      ...
      expect(config.nodeTestRunOptions).toEqual([
        '--experimental-test-coverage',
        '--test-reporter=spec',
      ]);
    });
    ...
    it('should resolve template placeholders in test name', () => {
      ...
      const patternArg =
        debugConfig.runtimeArgs![
          debugConfig.runtimeArgs!.indexOf('--test-name-pattern') + 1
        ];
