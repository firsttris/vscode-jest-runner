...
jest.mock('vscode', () => ({
  workspace: {
    getWorkspaceFolder: jest
      .fn()
      .mockReturnValue({ uri: { fsPath: '/mock/cwd' } }),
  },
  Uri: {
...
jest.mock('../execution/TestProcessRunner', () => ({
  executeTestCommand: jest
    .fn()
    .mockResolvedValue({ output: '', structuredResultsProcessed: false }),
...
describe('TestRunExecutor Bun Cleanup', () => {
  ...
  it('should delete previous coverage/lcov.info when running Bun with coverage', async () => {
    ...
    expect(fs.unlinkSync).toHaveBeenCalledWith(
      path.join('/mock/cwd', 'coverage', 'lcov.info'),
    );
  });
});
